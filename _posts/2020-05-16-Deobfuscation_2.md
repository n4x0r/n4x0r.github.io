---
layout: article
title:  "Tales Of Binary Deobfuscation - Part 2"
date:   2020-05-16 06:30
categories: article
tags: [reverse,malware, ELF, packers]
author: ulexec
---

On a [previous article](https://ulexec.github.io/ulexec.github.io/article/2020/03/03/Deobfuscation_1.html) We discussed how we can approach some obfuscation techniques such as intstruction substitution, opaque predicates or range dividers. This article is dedicated to cover control flow flattening and how we can leverage symbolic execution to attempt to deobfuscate it. A practical examples deobfuscating some of the latest implemented control-flow flattening in Emotet will be shown.

## Introduction 

There are various techniques to apply on the source-code level in order to make a subject application harder to analyze. Among the various existiing techniques, we can highlight control flow flattening, which will be the technique we will be covering in this post.

*'In the case of most applications, branches and their destinations are easily identifiable due to high level programming language constructs and coding guidelines. The complexity of determining the control flow of a function is linear with respect to the number of its basic blocks. The idea behind control flow flattening is to transform the structure of the source code in such a way that the branch destinations cannot be easily determined by static analysis, thus hindering the comprehension of the program'* - Tımea Laszlo

The basic method for flattening a function is the following:
1. Break up the body of a given function to basic blocks independenly of their different nesting levels. 

2. All of these basic blocks are encapsulated in a selective structure (a switch statement in the C++ language) with each block in a separate case, and the
selection is encapsulated in turn in a loop. 
3. The correct control flow of execution is ensured by a control or state variable representing the state of the program. This state variable will be modified on each basic block in order to define the next block to execute.

A visual representation of this technique is show in the figure below:

<br/>
<div style="text-align:center"><img src ="{{ site.url }}/images/tigress-cfg.png" /></div>
<br/>

As we can see in the figure above, the diagram of the left would be an unflatten control-flow diagram, while the one in the right will be the same control-flow diagram but flatten.
There are different variations of this technique, and really there is no a generic way to tackle all of them.

In this article we will focus on documenting different approaches for deobfuscating this technique using symbolic execution, and what are the advantages and limitations of each approach.

DISCLAIMER: This post is not meant to be a reference for deobfuscating any of the obfuscation schemes of the following malware families, rather is an attempt to share what methodologies I came across in order to tackle these techniques, but they are not perfect or generic and there may be more suitable techniques to approach control flow flattening that I haven't explore. You have been warned.

<br/>

## Emotet

Emotet also known as Geodo and Mealybug is a malware strain first detected in 2014 and remains active, deemed one of the most prevalent threats of 2019.
First versions of the Emotet malware functioned as a banking trojan aimed at stealing banking credentials from infected hosts. Throughout 2016 and 2017, Emotet operators updated the trojan and reconfigured it to work primarily as a "loader," a type of malware that gains access to a system, and then allows its operators to download additional payloads. On February 2020 it was [noticed](https://twitter.com/D00RT_RM/status/1225477850029084673) that newer Emotet samples had integrated control-flow flattening.
In this section we will be covering a methodology on how we can deobfuscate Emotet's specific control-flow flattening implementation
Specifically we will be looking at the following sample:

<br/>

``` http 
Emotet: 5670fc9e861dcf22cb0a63a53cddd40050819fe3f2f1e664f15412602d3b59e4
```

<br/>

If we observe the sample's main function we would see the following:

<br/>
<div style="text-align:center"><img src ="https://github.com/ulexec/ulexec.github.io/raw/master/images/maze-main.jpg" /></div>
<br/>

The preliminar impression can be overwhelming, however if we analyze the sample a bit more throughly we can derive a few conclusions.

* There are not API calls until the end of the function. 
* The state variable is hold within ebx
* We can identify the dispatcher easily by spotting the block with the biggest number of references

<br/>
<div style="text-align:center"><img src ="https://github.com/ulexec/ulexec.github.io/raw/master/images/maze-dispatcher.png" /></div>
<br/>

Furthermore, the dispacher compares the value of ebx to specific ones, and if equal it will branch to specific blocks. A brief snipet of the dispacher is as follows:
...
.text:0040668E                 sub     edi, edx
.text:00406690                 inc     edi
.text:00406691                 inc     ax
.text:00406693                 not     eax
.text:00406695                 and     edx, 0F64h
.text:0040669B                 mov     dx, di
.text:0040669E                 xor     edi, 0E23h
.text:004066A4                 inc     edx
.text:004066A5                 cmp     ebx, 1503h  
.text:004066AB                 jz      loc_4079DB           ; Branch to block for specific state
.text:004066B1                 not     cl
.text:004066B3                 dec     dx
.text:004066B5                 xor     esi, edx
.text:004066B7                 not     dh
.text:004066B9                 sub     dx, 405h
.text:004066BE                 or      ecx, edi
.text:004066C0                 sub     edi, 23CEh
.text:004066C6                 not     dh
.text:004066C8                 cmp     ebx, 1107h
.text:004066CE                 jz      loc_407AE9           ; Branch to block for specific state
...
...
if state variable has a specific value, it will jump to its correspondent block to then update the value of the state variable and jump back to the dispatcher:
...
...
.text:00407A97 loc_407A97:                            
.text:00407A97                 mov     ebx, 1
.text:00407A9C                 add     eax, 1B05h
.text:00407AA1                 xor     edx, ecx
.text:00407AA3                 inc     si
.text:00407AA5                 test    ebx, ebx
.text:00407AA7                 jz      short loc_407AAD
.text:00407AA9                 mov     cx, 1501h
.text:00407AAD
.text:00407AAD loc_407AAD:                            
.text:00407AAD                 mov     ebx, 122Eh           ; Updating state variable  
.text:00407AB2                 push    offset dispatcher
.text:00407AB7                 retn                         ; Jumping back to dispatcher
...
...
Based on the atomic structure of Maze's control-flow flattening implementation, seems that all relevant blocks per state value are located in blocks prior to jumping back to the dispatcher. We can attempt to simplify the control flow by using the following algorithm:

1. Locate the main function address, the address of the first block containing Windows API calls and the address of the dispatcher.
2. Find a feasible execution path from the beginning of the main function into first block containing Windows API's calls as destination address via Symbolic Execution.
3. After path is found, iterate through all symbolically executed blocks, and create a chain of 'relevant blocks' composed of predecessor blocks prior to the dispatcher.
4. Apply patches accordingly to simplify control-flow.

<br/>

We can implement this algorithm with any symbolic execution engine. For sake of simplicity, I chosed to implement this with angr:
...
def unflatten_maze_cfg(file_name, start_addr, clean_bbl_chain):
    r2 = r2pipe.open(file_name, ["-w"])

    r2.cmd("s %s" % start_addr)
    r2.cmd('wa jmp %s' % hex(clean_bbl_chain[0]))
    r2.cmd("so +1")

    for n in range(len(clean_bbl_chain) -1):
        r2.cmd('s %s' % hex(clean_bbl_chain[n]))
        
        while True:
            opcode_type = r2.cmdj('pdj 1')[0]['type']
            if ('jmp' in opcode_type and opcode_type is not 'cjump') or 'ret' in opcode_type:
                r2.cmd('wa jmp %s' % hex(clean_bbl_chain[n+1]))
                break
            r2.cmd('so +1')


def main():
    file_name = "./chacha.exe"
    main_function = 0x0405dc0
    target_addr = 0x0407CA6 
    dispatcher_node = 0x405DC5
    p = angr.Project("./chacha.exe")

    e = p.factory.blank_state(addr=main_function)
    cfg = p.analyses.CFGFast()
   
    pg = p.factory.simulation_manager(e)
    ...
    print('[+] Found Path!')
    print('[-] Executed Blocks: %s' % pg.found[0].history.block_count)

    executed_blocks = pg.found[0].history.bbl_addrs.hardcopy 
    clean_bbl_chain = []

    for n in range(len(executed_blocks)):
        if executed_blocks[n] == dispatcher_node:
            r_node = executed_blocks[n-2]
            clean_bbl_chain.append(r_node)
    clean_bbl_chain.append(target_addr)

    unflatten_maze_cfg(file_name, main_function, clean_bbl_chain)
...
After executing the previous script, we can see the before and after effects of the simplification. This is shown below:

<br/>

<br/>
<div style="text-align:center"><img src ="https://github.com/ulexec/ulexec.github.io/raw/master/images/maze-simplified.png" /></div>
<br/>
